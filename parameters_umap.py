...
for r in REPULSION_STRENGHT:
    reducer = umap.UMAP(repulsion_strength=r) 
    embedded = reducer.fit_transform(X_scaled)
    fig, ax = plt.subplots(figsize=(12, 10))
    scatter = ax.scatter(embedded[:, 0], embedded[:, 1],s=5,alpha=0.5)
    ax.set_title(f"UMAP using repulsion_strength={r}")
    ax.set_xlabel("UMAP 1")
    ax.set_ylabel("UMAP 2")

    plt.tight_layout()
    plt.savefig(f"umap_par/UMAP_repulsion_strength={r}.pdf")
...
for r in REPULSION_STRENGHT:
    ...
    plt.tight_layout()
    plt.savefig(f"umap_par/norm_UMAP_repulsion_strength={r}.pdf")
